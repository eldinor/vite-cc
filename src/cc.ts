...
export class CharacterController {
    ...
    public setGreetingAnim(
        rangeNames: AnimationGroup,
        rate: number,
        loop: boolean
    ) {
        this._greetingAG = {
    ...
    public setWalkBackAnim(
        ...
        this._copySlowAnims(
            this._actionMap.walkBackFast,
            this._actionMap.walkBack
        );
    }
    ...
    private _moveAVandCamera() {
        ...
        if (this._act._jump && !this._inFreeFall) {
            ...
            if (this._act._greeting) {
                actData = this._actionMap.greeting;
            } else {
            ...
            if (this._prevActData !== actData) {
                if (actData.exist) {
                    ...
                    if (this._isAG) {
                        if (
                            this._prevActData != null &&
                            this._prevActData.exist
                        )
                            this._prevActData.ag.stop();
                        actData.ag.start(actData.loop, actData.rate);
                        fps =
                            actData.ag.targetedAnimations[0].animation
                                .framePerSecond;
        ...
        if (this._cameraElastic) {
            if (pis.length > 0) {
                ...
                if (
                    ...
                    (!pis[0].pickedMesh.checkCollisions ||
                        !this._camera.checkCollisions)
                )
                    return;
                ...
                if (this._camera.checkCollisions) {
                    ...
                    if (l <= 1) {
                        ...
                        step = c2p
                            .normalize()
                            .scaleInPlace(l / this._elasticSteps);
                    }
                    this._camera.position =
                        this._camera.position.subtract(step);
                } else {
                    let step: number;
    ...
    private _onKeyDown(e: KeyboardEvent) {
        ...
        switch (e.key.toLowerCase()) {
            ...
            case this._actionMap.strafeRight.key:
                ...
                break;
            case this._actionMap.greeting.key:
                if (this._greetingAG) {
                    this._act.reset();
                    this._act._greeting = true;

                    const ags = this._greetingAG.ags;

                    this._actionMap.greeting = new ActionData(
                        this._actionMap.greeting.id,
                        this._actionMap.greeting.speed,
                        this._actionMap.greeting.key
                    );

                    this._setAnim(
                        this._actionMap.greeting,
                        ags,
                        this._greetingAG.rate,
                        this._greetingAG.loop
                    );
                }
                break;
        }
...
export class ActionMap {
    ...
    public greeting = new ActionData("greeting", 0, "1");
...
export function resetAnimations(bodyNode: any, cc: CharacterController) {
    const ags = (bodyNode as any).animationGroups as AnimationGroup[];

    const idleAG = ags.find(
